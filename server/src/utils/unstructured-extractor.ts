...
/**
 * @module unstructured-extractor
 * Orchestrates OCR text extraction via the Unstructured.io API.
 * Handles job submission, polling, output download, and text assembly.
 */
...
/**
 * Extracts text from a file buffer using the Unstructured.io API.
 * Submits a job, polls until completion, downloads output, and assembles text.
 *
 * @param buffer - The raw file buffer to process
 * @param filename - The original filename (used for the multipart upload)
 * @returns Extracted plain text
 * @throws {AppError} 502 if the API key is missing, job fails, or no output is produced
 * @throws {AppError} 504 if the job times out
 */
...
async function submitUnstructuredJob(
  ...
  const requestData = JSON.stringify({ template_id: DEFAULT_TEMPLATE_ID });
...
async function pollUnstructuredJob(jobId: string): Promise<UnstructuredJobInfo> {
  ...
  throw new AppError(504, `Unstructured OCR job timed out after ${timeoutMs} ms.`);
}
...
function collectOutputTargets(
  ...
  for (const output of outputFiles) {
    if (output.file_id) {
      const target: { fileId: string; nodeId?: string } = { fileId: output.file_id };
      if (output.node_id) {
